...
********************************************************
RXTE ASM PHAII Data (:mod:`gdt.missions.rxte.asm.phaii`)
********************************************************
...
Read in a 3rd dwell:
    ...
    >>> dwell_file3 = RXTEMissionTable.get_dwell_file(t0+180)
...
slow, we can write out our merged data to a fits file for later use.
    ...
    >>> RxtePhaiiNoHeaders.write(phaii_ssc1_dw21to23, directory='.', filename='test_rxte_phaii_ssc1_dw21to23.phaii')
    >>> RxtePhaiiNoHeaders.write(phaii_ssc2_dw21to23, directory='.', filename='test_rxte_phaii_ssc2_dw21to23.phaii')
    >>> RxtePhaiiNoHeaders.write(phaii_ssc3_dw21to23, directory='.', filename='test_rxte_phaii_ssc3_dw21to23.phaii') 


We can read that data back in now or at a later time using the open_fits method:

    >>> new_test1 = RxtePhaiiNoHeaders.open_fits('test_rxte_phaii_ssc1_dw21to23.phaii', "ssc1", t0) 
    >>> new_test2 = RxtePhaiiNoHeaders.open_fits('test_rxte_phaii_ssc2_dw21to23.phaii', "ssc2", t0) 
    >>> new_test3 = RxtePhaiiNoHeaders.open_fits('test_rxte_phaii_ssc3_dw21to23.phaii', "ssc3", t0) 
