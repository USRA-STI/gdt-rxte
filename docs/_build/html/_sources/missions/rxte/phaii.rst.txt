...
cameras (ssc0, ssc1, and ssc2) stored as ascii text files. Each file contains an
...
Now we are ready to fill the phaii class with the data:
    ...
    >>> phaii_ssc0_970815_dw21 = RxtePhaiiNoHeaders.open_ascii(dwell_file,"ssc0",t0)
...
Read in the next dwell (t0+90s):
    >>> dwell_file2 = RXTEMissionTable.get_dwell_file(t0+90,<path to rxte data>/camera_data')
    >>> phaii_ssc0_970815_dw22 = RxtePhaiiNoHeaders.open_ascii(dwell_file2,"ssc0",t0)
...
Read in a 3rd dwell:
    >>> dwell_file3 = RXTEMissionTable.get_dwell_file(t0+180,<path to rxte data>/camera_data')
    >>> phaii_ssc0_970815_dw23 = RxtePhaiiNoHeaders.open_ascii(dwell_file3,"ssc0",t0)
...
Now we can merge the three dwells:
    >>> phaii_ssc0_dw21to23 = RxtePhaiiNoHeaders.merge([phaii_ssc0_970815_dw21,phaii_ssc0_970815_dw22,phaii_ssc0_970815_dw23])
...
We make a plot for SSC0.    
    
    >>> lcplot = Lightcurve(phaii_ssc0_dw21to23.to_lightcurve())
    >>> plt.show()

This plot shows the count rates from t0 to t0+180s for SSC0

.. image:: phaii_figs/phaii_ssc0_fig1.png

Next we plot data for SSC1 
...
slow, we can write out our merged data to a fits file for later use.
    ...
    >>> RxtePhaiiNoHeaders.write(phaii_ssc0_dw21to23,directory='<my directory>',filename='test_rxte_phaii_ssc0_dw21to23.phaii',overwrite="True")
    >>> RxtePhaiiNoHeaders.write(phaii_ssc1_dw21to23,directory='<my_directory>',filename='test_rxte_phaii_ssc1_dw21to23.phaii',overwrite="True")
...
We can read that data back in now or at a later time using the open_fits method:
    ...
    >>> new_test0 = RxtePhaiiNoHeaders.open_fits('<my directory>/test_rxte_phaii_ssc0_dw21to23.phaii',"ssc0",t0) 
...
.. automodapi:: gdt.missions.rxte.phaii
   :inherited-members:
